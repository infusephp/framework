...
use app\statistics\models\StatisticSnapshot;
...
class SiteStats
{
	...
	static function generateSnapshot()
	{
		...
		// site status
		$return['site']['status'] = !Config::get( 'site.disabled' );

		// site mode
		$return['site']['mode'] = Config::get( 'site.production-level' );
		
		// session adapter
		$return['site']['session'] = Config::get( 'session.adapter' );		
