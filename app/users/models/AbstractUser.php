...
abstract class AbstractUser extends \infuse\Model
{
	...
	*/
	...
	{
		if( $this->id > 0 )
			return 'http://' . Config::get( 'site.host-name' ) . '/users/' . Util::seoify( $this->name( true ) . '-' . $this->id );
	...
	*/
	...
	static function createTemporary( $data )
	{
		...
		$insertArray = array_replace( $data, array(
			'registered_on' => time(),
			'enabled' => 0 ) );
	...
	function preSetHook( &$data )
	{
		...
		if( isset( $data[ 'current_password' ] ) && (
			Util::encrypt_password( $data[ 'current_password' ], Config::get( 'site.salt' ) ) == $this->get( 'user_password' ) ||
			static::$currentUser->isAdmin() ) )
		{
	...
	*/
	 ...
	 * @return boolean success?
	 */
	static function forgotStep1( $email, $ip )
	{
	...
	static function checkLogin( $username, $password )
	{
		...
		$user = static::findOne( array(
			'where' => array(
				$usernameWhere,
				'user_password' => Util::encrypt_password( $password, Config::get( 'site.salt' ) ) ) ) );
	...
	*/
	 ...
	 * @return boolean success?
	 */
	function deleteConfirm( $password, $req )
	{
		ErrorStack::setContext( 'user.delete' );
	
		// check for the confirm and password
		// only the current user can delete their account
		if( $this->id > 1 && static::currentUser()->id() == $this->id && Util::encrypt_password( $password, Config::get( 'site.salt' ) ) == $this->get( 'user_password' ) )
		{
			// delete the user
			Database::delete(
				'Users',
	...
	function sendEmail( $message, $details = array() )
	{
		...
		try
		{
			...
			$message = \Swift_Message::newInstance( $subject )
			  ->setFrom( array( Config::get( 'smtp.from' ) => Config::get( 'site.title' ) ) )
		...
		else if( $cookie = $req->cookies( 'persistent' ) )
		{
			...
			if( $cookieParams )
			{
				...
				if( $user )
				{
					...
					$seriesEnc = Util::encrypt_password( $cookieParams->series, Config::get( 'site.salt' ) );
					$tokenEnc = Util::encrypt_password( $cookieParams->token, Config::get( 'site.salt' ) );
